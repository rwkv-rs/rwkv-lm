"""TorchTitan tokenization and stateful data loading for RWKV-7."""
...
from rwkv_lm.binidx import MMapIndexedDataset
...
class RwkvTokenizer(BaseTokenizer):
    """Minimal deterministic tokenizer for synthetic and pre-tokenized training."""
    ...
    def encode(
        ...
        *,
        ...
        tokens = [1 + byte % (self.vocab_size - 1) for byte in text.encode("utf-8")]
        if add_bos:
            tokens.insert(0, 0)
        if add_eos:
            tokens.append(0)
...
__all__ = ["RwkvDataLoader", "RwkvTokenizer"]
