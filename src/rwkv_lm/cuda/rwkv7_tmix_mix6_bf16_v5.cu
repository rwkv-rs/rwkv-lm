...
__global__ void tmix_mix6_state_forward_kernel(
    const at::BFloat16* __restrict__ x,
...
__global__ void tmix_mix6_state_backward_fused_kernel_v5(
    const at::BFloat16* __restrict__ grad_r,
    const at::BFloat16* __restrict__ grad_w,
    const at::BFloat16* __restrict__ grad_k,
    const at::BFloat16* __restrict__ grad_v,
    ...
    for (int64_t bt = bt_start; bt < bt_end; ++bt) {
        ...
        if (have_next_grad) {
            ...
            dx_grad = __hadd2(dx_grad, load_bf16x2(grad_new_shift + b * c_size + c));
        }
        store_bf16x2(grad_x + idx, dx_grad);
        ...
        if (t == 0) {
            ...
            __nv_bfloat162 dshift = __floats2bfloat162_rn(0.0f, 0.0f);
            dshift = __hadd2(dshift, __hmul2(gr, pr));
            dshift = __hadd2(dshift, __hmul2(gw, pw));
            dshift = __hadd2(dshift, __hmul2(gk, pk));
            dshift = __hadd2(dshift, __hmul2(gv, pv));
            dshift = __hadd2(dshift, __hmul2(ga, pa));
            dshift = __hadd2(dshift, __hmul2(gg, pg));
            store_bf16x2(grad_shift_state + b * c_size + c, dshift);
        } else {
            x_prev = have_prev_x ? prev_x : __bfloat1622float2(load_bf16x2(x + idx - c_size));
        }
        float dx0 = x_prev.x - x_now.x;
...
std::vector<torch::Tensor> tmix_mix6_state_forward_v5_cuda(
    ...
    const int64_t total_pairs = bt_size * (c_size / 2);
    const int blocks = static_cast<int>(ceil_div(total_pairs, static_cast<int64_t>(threads)));
    tmix_mix6_state_forward_kernel<<<blocks, threads, 0, stream>>>(
        x.data_ptr<at::BFloat16>(),
        ...
        c_size);
    C10_CUDA_KERNEL_LAUNCH_CHECK();
    return {out_r, out_w, out_k, out_v, out_a, out_g, new_shift};
}
...
std::vector<torch::Tensor> tmix_mix6_state_backward_v5_cuda(
    ...
    dim3 fused_blocks(
        ...
        static_cast<unsigned int>(ceil_div(bt_size, static_cast<int64_t>(TMIX_PARAM_BT_TILE))),
        1);
    tmix_mix6_state_backward_fused_kernel_v5<<<fused_blocks, TMIX_PARAM_THREADS, 0, stream>>>(
        grad_r.data_ptr<at::BFloat16>(),
        grad_w.data_ptr<at::BFloat16>(),
        grad_k.data_ptr<at::BFloat16>(),
        grad_v.data_ptr<at::BFloat16>(),
        grad_a.data_ptr<at::BFloat16>(),
        grad_g.data_ptr<at::BFloat16>(),
        grad_new_shift.data_ptr<at::BFloat16>(),
        x.data_ptr<at::BFloat16>(),
        shift_state.data_ptr<at::BFloat16>(),
        x_r.data_ptr<at::BFloat16>(),
        x_w.data_ptr<at::BFloat16>(),
        ...
        x_g.data_ptr<at::BFloat16>(),
        grad_x.data_ptr<at::BFloat16>(),
        grad_shift_state.data_ptr<at::BFloat16>(),
        grad_x_r_fp32.data_ptr<float>(),
        grad_x_w_fp32.data_ptr<float>(),
        grad_x_k_fp32.data_ptr<float>(),
        grad_x_v_fp32.data_ptr<float>(),
    ...
    return {grad_x, grad_shift_state, grad_x_r, grad_x_w, grad_x_k, grad_x_v, grad_x_a, grad_x_g};
}
