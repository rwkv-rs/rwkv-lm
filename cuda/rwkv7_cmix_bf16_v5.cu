...
__global__ void cmix_mix_state_forward_vec2_kernel(
    const at::BFloat16* __restrict__ x,
    const at::BFloat16* __restrict__ shift_state,
    const at::BFloat16* __restrict__ x_k,
    at::BFloat16* __restrict__ out,
    int64_t bt_size,
    int64_t t_size,
    ...
    __nv_bfloat162 x_now = load_bf16x2(x + idx);
    __nv_bfloat162 x_prev = load_bf16x2(shift_state + b * c_size + c);
    if (t > 0) {
        x_prev = load_bf16x2(x + idx - c_size);
    }
    __nv_bfloat162 mix = load_bf16x2(x_k + c);
...
template<int BT_TILE>
__global__ void cmix_mix_state_backward_fused_vec2_kernel(
    const at::BFloat16* __restrict__ grad_out,
    const at::BFloat16* __restrict__ grad_new_shift,
    const at::BFloat16* __restrict__ x,
    const at::BFloat16* __restrict__ shift_state,
    const at::BFloat16* __restrict__ x_k,
    at::BFloat16* __restrict__ grad_x,
    at::BFloat16* __restrict__ grad_shift_state,
    float* __restrict__ grad_x_k,
    int64_t bt_size,
    int64_t t_size,
    int64_t c_size) {
    ...
    for (int64_t bt = bt_start; bt < bt_end; ++bt) {
        ...
        if (have_next_grad) {
            ...
            dx = __hadd2(dx, load_bf16x2(grad_new_shift + b * c_size + c));
        }
        store_bf16x2(grad_x + idx, dx);
        ...
        float2 x_prev = make_float2(0.0f, 0.0f);
        if (t == 0) {
            x_prev = __bfloat1622float2(load_bf16x2(shift_state + b * c_size + c));
            store_bf16x2(grad_shift_state + b * c_size + c, __hmul2(grad, mix));
        } else {
            x_prev = have_prev_x ? prev_x : __bfloat1622float2(load_bf16x2(x + idx - c_size));
        }
        float2 g = __bfloat1622float2(grad);
...
template<int BT_TILE>
void launch_cmix_mix_state_backward_fused_vec2_kernel(
    const at::BFloat16* grad_out,
    const at::BFloat16* grad_new_shift,
    const at::BFloat16* x,
    const at::BFloat16* shift_state,
    const at::BFloat16* x_k,
    at::BFloat16* grad_x,
    at::BFloat16* grad_shift_state,
    float* grad_x_k,
    int64_t bt_size,
    int64_t t_size,
    int64_t c_size,
    ...
    dim3 blocks(
        ...
        static_cast<unsigned int>(ceil_div(bt_size, static_cast<int64_t>(BT_TILE))),
        1);
    cmix_mix_state_backward_fused_vec2_kernel<BT_TILE><<<blocks, CMIX_THREADS, 0, stream>>>(
        grad_out,
        grad_new_shift,
        x,
        shift_state,
...
std::vector<torch::Tensor> cmix_mix_state_forward_cuda(torch::Tensor x, torch::Tensor shift_state, torch::Tensor x_k) {
    auto out = torch::empty_like(x);
    auto new_shift = x.select(1, x.size(1) - 1).contiguous();
    const int64_t bt_size = x.size(0) * x.size(1);
    const int64_t c_size = x.size(2);
    const int64_t total_pairs = bt_size * (c_size / 2);
    auto stream = at::cuda::getCurrentCUDAStream();
    const int blocks = static_cast<int>(ceil_div(total_pairs, static_cast<int64_t>(CMIX_THREADS)));
    cmix_mix_state_forward_vec2_kernel<<<blocks, CMIX_THREADS, 0, stream>>>(
        x.data_ptr<at::BFloat16>(),
        shift_state.data_ptr<at::BFloat16>(),
        x_k.data_ptr<at::BFloat16>(),
        out.data_ptr<at::BFloat16>(),
        bt_size,
        x.size(1),
...
std::vector<torch::Tensor> cmix_mix_state_backward_cuda(
    ...
    torch::Tensor x_k) {
    auto grad_x = torch::empty_like(x);
    auto grad_shift_state = torch::empty_like(shift_state);
    auto grad_x_k_fp32 = torch::zeros({x.size(2)}, x.options().dtype(torch::kFloat32));
    auto grad_x_k = torch::empty({x.size(2)}, x.options());
    ...
    if (bt_tile == 16) {
        launch_cmix_mix_state_backward_fused_vec2_kernel<16>(
            grad_out.data_ptr<at::BFloat16>(), grad_new_shift.data_ptr<at::BFloat16>(),
            x.data_ptr<at::BFloat16>(), shift_state.data_ptr<at::BFloat16>(), x_k.data_ptr<at::BFloat16>(),
            grad_x.data_ptr<at::BFloat16>(), grad_shift_state.data_ptr<at::BFloat16>(),
            grad_x_k_fp32.data_ptr<float>(), bt_size, x.size(1), c_size, stream);
    } else if (bt_tile == 64) {
        launch_cmix_mix_state_backward_fused_vec2_kernel<64>(
            grad_out.data_ptr<at::BFloat16>(), grad_new_shift.data_ptr<at::BFloat16>(),
            x.data_ptr<at::BFloat16>(), shift_state.data_ptr<at::BFloat16>(), x_k.data_ptr<at::BFloat16>(),
            grad_x.data_ptr<at::BFloat16>(), grad_shift_state.data_ptr<at::BFloat16>(),
            grad_x_k_fp32.data_ptr<float>(), bt_size, x.size(1), c_size, stream);
    } else if (bt_tile == 128) {
        launch_cmix_mix_state_backward_fused_vec2_kernel<128>(
            grad_out.data_ptr<at::BFloat16>(), grad_new_shift.data_ptr<at::BFloat16>(),
            x.data_ptr<at::BFloat16>(), shift_state.data_ptr<at::BFloat16>(), x_k.data_ptr<at::BFloat16>(),
            grad_x.data_ptr<at::BFloat16>(), grad_shift_state.data_ptr<at::BFloat16>(),
            grad_x_k_fp32.data_ptr<float>(), bt_size, x.size(1), c_size, stream);
    } else {
        launch_cmix_mix_state_backward_fused_vec2_kernel<32>(
            grad_out.data_ptr<at::BFloat16>(), grad_new_shift.data_ptr<at::BFloat16>(),
            x.data_ptr<at::BFloat16>(), shift_state.data_ptr<at::BFloat16>(), x_k.data_ptr<at::BFloat16>(),
            grad_x.data_ptr<at::BFloat16>(), grad_shift_state.data_ptr<at::BFloat16>(),
            grad_x_k_fp32.data_ptr<float>(), bt_size, x.size(1), c_size, stream);
    ...
    return {grad_x, grad_shift_state, grad_x_k};
}
...
std::vector<torch::Tensor> cmix_layer_state_forward_v5_cuda(
    ...
    torch::Tensor key_weight,
    torch::Tensor value_weight) {
    auto mixed_state = cmix_mix_state_forward_cuda(x, shift_state, x_k);
    auto mixed = mixed_state[0];
    auto new_shift = mixed_state[1];
    auto mixed_2d = mixed.view({-1, mixed.size(2)});
    auto act = at::matmul(mixed_2d, key_weight.transpose(0, 1)).contiguous();
    relu_sq_inplace_cuda(act);
    auto out_2d = at::matmul(act, value_weight.transpose(0, 1));
    auto out = out_2d.view({x.size(0), x.size(1), value_weight.size(0)}).contiguous();
    return {out, mixed, act, new_shift};
}
...
std::vector<torch::Tensor> cmix_layer_state_backward_v5_cuda(
    ...
    auto grad_mixed_2d = at::matmul(grad_hidden, key_weight).contiguous();
    auto grad_mixed = grad_mixed_2d.view_as(x);
    auto mix_grads = cmix_mix_state_backward_cuda(grad_mixed, grad_new_shift, x, shift_state, x_k);
